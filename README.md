...
а). Рейтинг <= 3 - "плохой";
б). 3 < рейтинг <= 4 - "хороший";
в). 4 < рейтинг <= 5 - "отличный";
г). в иных случаях - "плохой".
...
а). Если не задано поле product_id, то берется значение предыдущего объекта, где оно было (при инициализации метода значение принимается равным 0);
б). Свойство price объекта Product парсится только в том случае, если в xml есть поле inet_price (т.е. tckb inet_price не пусто, то берем price).
в). После вычисления price расчитывается соответствующая ему категория.